...
public final class Run {
    ...
    private ArrayDeque<Page> pagesHistory;
...
//        this.setCurrentMovie(new Movie());
        this.setCurrentMovie(new Movie.Builder().build());
        this.setPagesHistory(new ArrayDeque<>());

        ArrayList<ActionInput> actions = inputData.getActions();
        for (ActionInput actionInput : actions) {
//            System.out.println(this.currentPage.getName());
            switch (actionInput.getType()) {
                case "change page" -> {
                    switch (actionInput.getPage()) {
                        ...
                        case "movies" -> database.movies(this, actionInput, output,
                                objectMapper);
                        case "see details" -> database.seeDetails(this, actionInput, output,
                                objectMapper);
                        case "Homepage autentificat" -> database.changePageAuthPage(this, output,
                                objectMapper);
                ...
                case "on page" -> {
                    switch (actionInput.getFeature()) {
                        ...
                        case "register" -> database.register(this, actionInput, output,
                                objectMapper);
                        case "search" -> database.movies(this, actionInput, output,
                                objectMapper);
                        case "filter" -> database.movies(this, actionInput, output,
                                objectMapper);
                        ...
                        case "buy premium account" -> database.upgrades(this, actionInput, output,
                                objectMapper);
                        case "subscribe" -> database.seeDetails(this, actionInput, output,
                                objectMapper);
                        default -> output.add(database.errorOutput(objectMapper));
                    }
                }
                case "back" -> database.back(this, output, objectMapper);
                case "database" -> {
                    switch (actionInput.getFeature()) {
                        case "add" -> database.add(actionInput, output, objectMapper);
                        case "delete" -> database.delete(actionInput, output, objectMapper);
                        default -> output.add(database.errorOutput(objectMapper));
                    }
                }
                default -> output.add(database.errorOutput(objectMapper));
        ...
        Recommendations recommendations = (Recommendations) actionFactory.createAction(
                ActionFactory.ActionType.Recommendations);
        recommendations.recomandation(this, database, output, objectMapper);
    }
     ...
     * @return
     */
    public ArrayDeque<Page> getPagesHistory() {
        return pagesHistory;
    }
     ...
     * @param pagesHistory
     */
    public void setPagesHistory(final ArrayDeque<Page> pagesHistory) {
